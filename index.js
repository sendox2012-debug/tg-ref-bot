...
const pf = {
  ...
  async request(endpoint, body) {
    if (!this.enabled()) throw new Error('PIARFLOW_API_KEY пуст');
    console.log(`🌐 PF Request: POST ${endpoint} | Payload:`, JSON.stringify(body));
    ...
    console.log(`🌐 PF Response ${res.status}:`, JSON.stringify(data));
    if (!res.ok) throw new Error(`PiarFlow ${res.status}: ${data.message || 'Error'}`);
    return data;
  },
  async getSponsors(uid, chatId) {
    if (!this.enabled()) return null;
    try {
      const d = await this.request('/sponsors', { user_id: Number(uid), chat_id: Number(chatId), max_sponsors: 10 });
      return Array.isArray(d.sponsors) ? d.sponsors : [];
    } catch (e) { console.error(`❌ PF getSponsors: ${e.message}`); return null; }
  },
  async checkSponsors(uid, links) {
    if (!this.enabled()) return null;
    try {
      const d = await this.request('/sponsors/check', { user_id: Number(uid), links });
      return Array.isArray(d.sponsors) ? d.sponsors : [];
    } catch (e) { console.error(`❌ PF checkSponsors: ${e.message}`); return null; }
  }
};
...
async function startBotSafely() {
  try {
    ...
    if (err?.error_code === 409) { console.error('❌ 409 Conflict.'); process.exit(1); }
...
bot.command('test_pf', async (ctx) => {
  ...
  const uid = ctx.from.id;
  ctx.reply(`🧪 Тест PiarFlow для ${uid}...`);
  const sp = await pf.getSponsors(uid, ctx.chat.id);
  await ctx.reply(`📡 /sponsors:\n<code>${JSON.stringify(sp, null, 2)}</code>`, { parse_mode: 'HTML' });
  if (sp?.length) {
    const chk = await pf.checkSponsors(uid, sp.map(s => s.link));
    await ctx.reply(`📡 /check:\n<code>${JSON.stringify(chk, null, 2)}</code>`, { parse_mode: 'HTML' });
  }
});
